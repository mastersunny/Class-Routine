		...
		<div class="container">
			...
			<div class="collapse navbar-collapse" id="myNavbar">
				...
				<h3 class="navbar-text text-primary"
					style="position: relative; left: 31%;">Routine: ${department}</h3>

				<ul class="nav navbar-nav navbar-right">
					<li><a
						href="${pageContext.request.contextPath}/admin/createroutine?employeeCode=${teacher.employeeCode}">Create
							Routine</a></li>
					<li><a id="logOut"
		...
		<div class="row" style="margin-bottom: 10px;">
			...
			<div class="col-md-6 col-md-offset-3"
				...
				<ul class="nav nav-pills">
					<li><a data-toggle="pill" href="#semester1">semester 1-1</a></li>
					<li><a data-toggle="pill" href="#semester2">semester 2-1</a></li>
					<li><a data-toggle="pill" href="#semester3">semester 3-1</a></li>
					<li><a data-toggle="pill" href="#semester4">semester 4-1</a></li>
				</ul>
			</div>
		</div>
									...
									<c:if test="${routine.dayId.dayId==i && routine.startTime==j}">
										<c:choose>
											<c:when test="${(routine.endTime-routine.startTime)==1}">
												...
												<div class='col-xs-1 hour drag clickable'
											...
											<c:when test="${(routine.endTime-routine.startTime)==2}">
												...
												<div class='col-xs-2 hour drag clickable'
													data-dept="${teacher.deptId.deptId}"
													data-teaches="${routine.teachesId.teachesId}"
													data-sem="${com1}" data-day="${i}"
													data-id="${routine.mainRoutineId}" data-time="${j}"
											...
											<c:when test="${(routine.endTime-routine.startTime)==3}">
												<div class='col-xs-3 hour drag clickable'
													data-dept="${teacher.deptId.deptId}"
													data-teaches="${routine.teachesId.teachesId}"
													data-sem="${com1}" data-day="${i}"
													data-id="${routine.mainRoutineId}" data-time="${j}"
		...
		$(document)
				.ready(
						function() {
							...
							var dayId, startTime, deptId, examCommitteeId, teachesId, time, mainRoutineId, roomId, currentHour, newHour, currentDayId, currentStartTime, currentExamCommitteeId, count = 0;
							...
							var com1 = $('#semester1').data('sem'), com2 = $(
									'#semester2').data('sem'), com3 = $(
									'#semester3').data('sem'), com4 = $(
									'#semester4').data('sem');
																		...
																		for (var i = 0; i < obj.length; i++) {
																			...
																			if (roomId == obj[i].roomId) {
																				...
																				for (var j = 1; j <= obj[i].hour; j++) {
																					...
																					items += "<label class='radio-inline'><input type='radio' name='hour' value="+j+">"
																							+ j
																							+ "</label>";
																				}
							...
							function init() {
								...
								function myHelper(event) {
									...
									if ($(this).data('new') == "newCourse") {
										...
										var $this = $(this).children('p').eq(0)
												.text();
										string = $this.split(":");
										return '<h2 style="font-weight:bold;font-family:sans-serif;">'
												+ string[1] + '</h2>';

									}
									var $this = $(this).find('ul').children(
											'li').eq(0);
									return '<h2 style="font-weight:bold;font-family:sans-serif;">'
											+ $this.text() + '</h2>';
								}
								...
								function handleDropEvent(event, ui) {
									...
									dayId = $(this).data('day'), time = $(this)
											.data('time'), deptId = $(this)
											.data('dept');
									...
									courseCode = dragged.find('ul').children(
											'li').eq(0).text();
									room = dragged.find('ul').children('li')
											.eq(1).text();
									teacher = dragged.find('ul').children('li')
											.eq(2).text();
									...
									currentExamCommitteeId = dragged
											.data('sem');
							...
							function click() {
								...
								var title = $('.modal-title').children('h4')
										.eq(0);
								var myModal = $('#myModal');
								...
								clickable
										.dblclick(function() {
											...
											if ($this.find('ul').length > 0) {
												...
												var courseCode = $this.find(
														'ul').children('li')
														.eq(0).text();
												var room = $this.find('ul')
														.children('li').eq(1)
														.text();
												var teacher = $this.find('ul')
														.children('li').eq(2)
														.text();
												...
												$('#currentCourse').empty()
														.append(ul);
												title
														.text("UPDATE OR DELETE A COURSE");
												$('#delete').show();
												$('#myModal').modal('show');
												...
												currentDayId = $this
														.data('day');
												currentStartTime = $this
														.data('time');
												currentExamCommitteeId = $this
														.data('sem');
																...
																if (data == "hourlessthan") {
																	...
																	$(
																			...
																			.eq(
																					1)
																			.text(
																					"Given Hour is greater than available course hour");
																	count++;
							...
							function newTeachesList(data) {
								...
								if (data == null) {
									id
											.append("<div style='background-color:#FFFFFF;'><p>You have no courses available to add<p></div>");
									return;
								}
			          ...
			          + data[i].teachesId +"><p style='font-weight:bold;font-family:sans-serif;'>Course Code: "
											+ data[i].courseCode
											+ "</p><p style='font-family:sans-serif;'>Available Hours: "
											+ data[i].hour + "</p></div>");
